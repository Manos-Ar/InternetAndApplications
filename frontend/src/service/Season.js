...
class Season extends Component{
  ...
  handleSubmit(e) {
    ...
    // fetch('/energy/api/'+Dataset+'/'+AreaName+'/'+Resolution+'/'+SearchPeriod+'/'+Date,{
    // fetch("http://localhost:8080/NBA/"+Year+'/'+Position+'/'+Criteria+'/'+Size,{
    ...
    fetch(url+Year+'/'+Position+'/'+Criteria+'/'+Size,{
      method: 'GET',
      // headers: {'X-OBSERVATORY-AUTH': this.context.token}
    })
...
// 			if (URLError){
// 				return (<div class="alert"><h1><strong>Error: </strong>Fill all fields</h1> Refresh the page.</div>);
// }
// 			else {
				return(
    ...
    <button class="button button2" type="submit">Search</button>
		...
		{(InvalidYear)?<p>Invalid Year Range.
			Choose between 1950-2017</p>:<div/>}
		{(isSubmitted)
			?(
